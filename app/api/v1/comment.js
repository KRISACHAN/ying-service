...
// 删除评论
router.delete('/comment/:id', userAuthMiddleware, async ctx => {
    // 通过验证器校验参数是否通过
    const v = await new PositiveArticleIdParamsValidator().validate(ctx);

    // 获取分类ID参数
    const id = v.get('path.id');
    const [err] = await CommentDao.destroy(id);
    if (!err) {
        // 返回结果
        ctx.response.status = 200;
        ctx.body = resolve.success('删除评论成功');
    } else {
        ctx.body = resolve.fail(err);
    }
});
...
// 修改评论
router.put('/comment/:id', userAuthMiddleware, async ctx => {
    // 通过验证器校验参数是否通过
    const v = await new PositiveArticleIdParamsValidator().validate(ctx);

    // 获取分类ID参数
    const id = v.get('path.id');
    const [err] = await CommentDao.update(id, v);
    if (!err) {
        // 返回结果
        ctx.response.status = 200;
        ctx.body = resolve.success('更新评论成功');
    } else {
        ctx.body = resolve.fail(err);
    }
});
...
// 获取评论详情
router.get('/comment/:id', userAuthMiddleware, async ctx => {
    // 通过验证器校验参数是否通过
    const v = await new PositiveArticleIdParamsValidator().validate(ctx);
