...
import { isArray, unique } from '@utils/helpers';
...
export class CommentDao {
    ...
    static async _handleReply(comment) {
        try {
            ...
            const commentIds = isArrayData
                ? unique(comment.map(c => c.id))
                : comment.id;
            const [replyErr, replyData] = await CommentDao.getReplyData(
                commentIds,
            );
            ...
            if (!replyErr) {
                ...
                const [userErr1, userData1] = await CommentDao.getUserData(
                    newUserIds,
                );
                if (!userErr1) {
                ...
                return CommentDao._setCommentByDataValue(
                    comment,
                    replyData,
                    'id',
                    'reply_list',
            ...
            Object.keys(data).forEach(key => {
                ...
                if (isArray(item)) {
                    ...
                    if (item.reply_user_id) {
                        item[REPLY_USER_INFO] =
                            user[item.reply_user_id] || null;
                    }
                }
            });
    ...
    static async _handleArticle(comment) {
        ...
        const articleIds = isArrayData
            ? unique(comment.map(c => c.article_id))
            : comment.article_id;

        // 进行查询
        const [articleErr, articleData] = await CommentDao.getArticleData(
            articleIds,
        );
